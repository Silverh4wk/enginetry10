...
struct dsound_Sound_Output 
{
	...
	int ToneHz ; // number of cycles per second 
	...
	LPARAM lParam) 
{
	...
	// (Note: Silver) -- Much more work need to be done here 
	...
	case WM_KEYUP:
	{
		...
		bool AltKeyIsDown = ((lParam & (1 << 29)) != 0);
			...
			else if ((VKCode == VK_F4 ) and AltKeyIsDown) {
				GlobalRunning = false;
...
void win32FillSoundBuffer(dsound_Sound_Output* SoundOutput, DWORD ByteToLock, DWORD WriteBytes)
{
	VOID* Region;
	DWORD RegionSize;
	VOID* Region2;
...
int CALLBACK WinMain(HINSTANCE Instance, HINSTANCE PrevInstance, LPSTR CmdLine, int showCode)
{
	...
	if (RegisterClassExA(&WindowClass)) {
		...
		if (Window)
		{
			...
			SoundOutput.ToneHz = 256; // number of cycles per second 
